...
handle_mount() {
    ...
    # Check if directory is already in /etc/fstab
    if grep -q "10.0.0.1:$dir" /etc/fstab; then
    ...
    if [ "$already_mounted" = false ] && [ "$in_fstab" = false ]; then
        ...
        echo "Creating $dir mount point..."
        sudo mkdir -p $dir

        # Mount the remote NFS directory
        echo "Mounting $dir from 10.0.0.1..."
        ...
        echo "No changes were made for $dir."
    fi
}
